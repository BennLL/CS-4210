...
# AUTHOR: Zihao Luo
# FILENAME: cnn.py
# SPECIFICATION: This program answers the question of  Complete the Python program (cnn.py) that will build and train a Convolutional Neural  Network (CNN) using TensorFlow and Keras to classify 32x32 images of handwritten digits (0 through  9). Your task is to 1) load the dataset using the provided helper function, 2) normalize and reshape the  input data for the CNN, 3) construct a CNN architecture following the guidelines in the code, 4) train  the model on the training set and validate on the test set, 5) evaluate the final model and 6) print the  test accuracy. 
...
# --> add your Python code here
X_train, Y_train = load_digit_images_from_folder('Assignment 4/train')
X_test, Y_test = load_digit_images_from_folder('Assignment 4/test')
